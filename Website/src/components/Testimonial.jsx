import React from 'react'
import Title from './Title'
import { testimonials } from '../assets/assets'
import StarRating from './StarRating'

const Testimonial = () => {
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 bg-slate-50 pt-20 pb-30 mb-20'>
      <Title title='What Our Guests Say' subTitle='Discover why discerning travelers consistently choose QuickStay for their exclusive and luxurious accommodations around the world.' />
      <div class="flex flex-wrap items-center justify-center gap-6 mt-20 mb-10">
        {testimonials.map((testimonial) => (
          <div key={testimonial.id} class="bg-white p-6 rounded-xl shadow max-w-xs">
            <div class="flex items-center gap-3">
              <img class="w-12 h-12 rounded-full" src={testimonial.image} alt={testimonial.name} />
              <div>
                <p class="text-xl font-playfair">{testimonial.name}</p>
                <p class="text-gray-500">{testimonial.address}</p>
              </div>
            </div>
            <div class="flex items-center gap-1 mt-4">
              <StarRating />
            </div>
            <p class="text-gray-500 max-w-90 mt-4">{testimonial.review}</p>
          </div>
        ))}
      </div>
    </div>
  )
}

export default Testimonial